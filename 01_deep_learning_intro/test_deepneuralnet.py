...
@author: maree
'''
from machine_learning import DeepNeuralNetClass 
import  numpy as np
import  pandas as pd
import random
'''
...
dnn_c = DeepNeuralNetClass( np.size(X, 1) );
...
for i in range(epochs+1):
    dnn_c.train(X, Y);
    if i % (epochs / 10) == 0:
        prediction,loss_epoch = dnn_c.predict(X, Y);
        epochs_list.append(i)
        losses.append(loss_epoch)

df_print( pd.DataFrame({'epoch' : epochs_list,'loss' : losses}) );
df_print( pd.DataFrame(data=np.concatenate([prediction, Y], axis=1),columns=['Prediction', 'Actual']) )
